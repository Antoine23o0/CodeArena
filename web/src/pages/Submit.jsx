...
export default function Submit() {
  ...
  const isBusy = isSubmitting || isCompiling;
  ...
  const compileCode = async () => {
    ...
    try {
      ...
      const { data } = await api.post("/submissions/compile", payload);
      setResult(data);
    } catch (err) {
      setError(err.response?.data?.error ?? "Échec de la compilation");
    } finally {
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                onClick={compileCode}
                disabled={isBusy}
                className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg font-medium disabled:opacity-60 disabled:cursor-not-allowed transition"
              >
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-6 shadow-lg space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold">
                {result.isCompilation ? "Résultat de la compilation" : "Résultat de la soumission"}
              </h2>
